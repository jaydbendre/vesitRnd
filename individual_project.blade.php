@extends('boilerplate')

@section('page')

<div class="container">
    <div class="jumbotron">
        <div class="row">
            <div class="col-md-3">
                    <img style="width:100px;" src="https://i.stack.imgur.com/34AD2.jpg" style="float:left;" class="img-rounded">
            </div>

            <div class="com-md-5">
                    <div class = "panel panel-success">
                            <div class = "panel-heading">
                               <h3 class = "panel-title">
                                  Information
                               </h3>
                            </div>
                            
                            <div class = "panel-body">
                               <table class="table table-striped" style="width:250px;padding-right:200px;">
                                   <tr>
                                       <td>Project Incharge</td>
                                       <td>Incharge</td>
                                   </tr>
                                   <tr>
                                        <td>Date</td>
                                        <td>Date</td>
                                    </tr>
                                    <tr>
                                            <td>Area of research</td>
                                            <td>Unknown</td>
                                        </tr>
                                         
                               </table>
                            </div>
                         </div>
            </div>

            <div class="com-md-4">
                    <div class = "panel panel-danger">
                            <div class = "panel-heading">
                               <h3 class = "panel-title">
                                  About Faculty
                               </h3>
                            </div>
                            
                            <div class = "panel-body">
                               <table class="table table-bordered" style="width:250px;padding-right:200px;">
                                   <tr>
                                       <td>Faculty name</td>
                                       <td>Name</td>
                                   </tr>
                                   <tr>
                                        <td>Area of research</td>
                                        <td>Still Unknown</td>
                                    </tr>
                                    <tr>
                                            <td>Projects</td>
                                            <td><u>Out of Scope<u></td>
                                        </tr>
                                         
                               </table>
                            </div>
                         </div>
            </div>

            
    </div>

    <div class = "panel panel-info">
            <div class = "panel-heading">
                <h3 class = "panel-title">
                    This is title
                </h3>
            </div>
            
            <div class = "panel-body">
               <h3 style="font-family:Arial, Helvetica, sans-serif">Description</h3>
               <br/><br/>
               <p>
                    Dependent certainty off discovery him his tolerably offending. Ham for attention remainder sometimes additions recommend fat our. 
                    Direction has strangers now believing. Respect enjoyed gay far exposed parlors towards. Enjoyment use tolerably dependent listening men.
                     No peculiar in handsome together unlocked do by. Article concern joy anxious did picture sir her. 
                    Although desirous not recurred disposed off shy you numerous securing. 


                    Dependent certainty off discovery him his tolerably offending. Ham for attention remainder sometimes additions recommend fat our. 
                    Direction has strangers now believing. Respect enjoyed gay far exposed parlors towards. Enjoyment use tolerably dependent listening men.
                     No peculiar in handsome together unlocked do by. Article concern joy anxious did picture sir her. 
                    Although desirous not recurred disposed off shy you numerous securing. 
                    
                    Dependent certainty off discovery him his tolerably offending. Ham for attention remainder sometimes additions recommend fat our. 
                    Direction has strangers now believing. Respect enjoyed gay far exposed parlors towards. Enjoyment use tolerably dependent listening men.
                     No peculiar in handsome together unlocked do by. Article concern joy anxious did picture sir her. 
                    Although desirous not recurred disposed off shy you numerous securing. 
                    
               </p>
            </div>
            </div>
</div>
</div>
</div>

@endsection